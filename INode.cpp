...
void INode::ReadI() {
	...
	while (User::U_NOERROR == u.u_error && u.IOParam.count) {
		...
		nbytes = min(INode::BLOCK_SIZE - offset /* 块内有效字节数 */, u.IOParam.count);
		...
		/* 缓存中数据起始读位置 */
		unsigned char* start = pBuffer->b_addr + offset;
...
void INode::WriteI() {
	...
	while (User::U_NOERROR == u.u_error && u.IOParam.count) {
		...
		offset = u.IOParam.offset % INode::BLOCK_SIZE;
		nbytes = min(INode::BLOCK_SIZE - offset, u.IOParam.count);
		if ((bn = this->Bmap(lbn)) == 0) {
			return;
		}
...
int INode::Bmap(int lbn) {
	...
	*/
	...
	Buf* pFirstBuffer, * pSecondBuffer;
	int phyBlkno, index;
	int* iTable;
	...
	if (phyBlkno) {
		...
		this->i_addr[index] = pFirstBuffer->b_blkno;
	}

	iTable = (int*)pFirstBuffer->b_addr;
	if (index >= 8) {
		/*
		...
		*/
		...
		if (phyBlkno) {
			...
			iTable[index] = pSecondBuffer->b_blkno;
			bufferManager.Bdwrite(pFirstBuffer);
		}

		pFirstBuffer = pSecondBuffer;
		iTable = (int*)pSecondBuffer->b_addr;
	}
	...
	if ((phyBlkno = iTable[index]) == 0 && (pSecondBuffer = fileSystem.Alloc()) != NULL) {
		phyBlkno = pSecondBuffer->b_blkno;
...
void INode::IUpdate(int time) {
	...
	if (this->i_flag & (INode::IUPD | INode::IACC)) {
		...
		unsigned char* p = pBuffer->b_addr + (this->i_number % FileSystem::INODE_NUMBER_PER_SECTOR) * sizeof(DiskINode);
