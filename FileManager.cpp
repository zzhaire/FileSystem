...
INode* FileManager::NameI(enum DirectorySearchMode mode) {
    ...
    while (1) {
        ...
        memset(u.dbuf, 0, sizeof(u.dbuf));
        memcpy(u.dbuf, u.dirp.data() + index, (nindex == (unsigned int)string::npos ? u.dirp.length() : nindex) - index);
        index = nindex + 1;
        ...
        while (1) {
            ...
            memcpy(&u.dent, pBuffer->b_addr + (u.IOParam.offset % INode::BLOCK_SIZE), sizeof(u.dent));
            ...
            if (!memcmp(u.dbuf, &u.dent.name, sizeof(DirectoryEntry) - 4)) {
                break;
            }
        }
...
void FileManager::WriteDir(INode* pINode) {
    ...
    /* 设置目录项中pathname分量部分 */
    memcpy(u.dent.name, u.dbuf, DirectoryEntry::DIRSIZ);
